...
public class AWSServiceWiseCostComparisonLast3months {
    WebDriver driver;
    ...
    public void ApiAmountComparisionwithPDFExtractedData() throws IOException {
        ...
        Response response = RestAssured
            ...
            .header("Authorization", "Bearer M/UrvGkGmpou50vVGN+Lk1OGnj1")
        ...
        try {            
            ...
            List<Object> groupValues = response.jsonPath().getList("data.costExplorerTableData.serviceSummary.findAll { it.groupValue == 'Amazon Elastic Compute Cloud' }.groupValue");

            List<Object> costDates = response.jsonPath().getList("data.costExplorerTableData.serviceSummary.findAll { it.groupValue == 'Amazon Elastic Compute Cloud' }.costDates.");
            List<Object> totalCost = response.jsonPath().getList("data.costExplorerTableData.serviceSummary.findAll { it.groupValue == 'Amazon Elastic Compute Cloud' }.totalCost.");

            // Convert List<Object> to List<String>
            List<String> costDatesStringList = costDates.stream()
                                                        .map(Object::toString) // Convert each Object to String
                                                        .collect(Collectors.toList());
            List<String> groupValueStringList = groupValues.stream()
                    .map(Object::toString) // Convert each Object to String
                    .collect(Collectors.toList());
            
            List<String> totalCostStringList = totalCost.stream()
                                                            .map(Object::toString) // Convert each Object to String
                                                            .collect(Collectors.toList());

            // Add both lists to expectedTexts
            expectedTexts.addAll(costDatesStringList);
            expectedTexts.addAll(groupValueStringList);
            expectedTexts.addAll(totalCostStringList);
